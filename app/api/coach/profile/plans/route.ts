...
export async function PUT(req: NextRequest) {
  ...
  const { error } = await supabase
    .from("profiles")
    .update({ plan_monthly_price, plan_monthly_due_day, plan_annual_price })
    .eq("id", user.id);

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ok: true, plan_monthly_price, plan_monthly_due_day, plan_annual_price });
}
